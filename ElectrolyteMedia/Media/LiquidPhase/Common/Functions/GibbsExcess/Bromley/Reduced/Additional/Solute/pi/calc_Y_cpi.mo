...
function calc_Y_cpi "Helper function to calculate Gibbs derivative"

  input SI.Temperature T;
  input SI.Pressure p;
  input SI.MassFraction[nLfun] X;

  output Real[nLfun-1] Y_cpi;
protected
    ...
    if datafun[i].z > 0 then
      for k in 1:nLfun-1 loop
        if datafun[k].z < 0 then
          Y_cpi[i] :=Y_cpi[i] + (abs(datafun[i].z) + abs(datafun[k].z))^2/4*mol_i[k]/I * loggamma_ik_0pi[i,k];
        end if;
      end for;
    end if;
  end for;
