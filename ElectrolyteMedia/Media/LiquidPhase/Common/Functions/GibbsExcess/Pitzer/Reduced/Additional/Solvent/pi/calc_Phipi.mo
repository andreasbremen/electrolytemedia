...
function calc_Phipi "Helper function to calculate Gibbs derivative"
...
protected
  Real m_S;
  Real A_Phi_lnpi = GibbsExcess.DebyeHueckel.Reduced.Additional.dg_dpi.calc_dAln_dpi(T,p);
  Real I = calc_I(X);
  ...
  Real[nLifun,nLifun] Phi_Phi_ijpi = calc_Phi_Phi_ijpi(T,p,X);
...
//   end for;
  ...
  Phi := -(A_Phi_lnpi*I^(3/2))/(1 + 1.2*I^(1/2));
  ...
  for i in 1:nLifun loop
    if datafun[i].z > 0 then
      for j in 1:nLifun loop
        if datafun[j].z > 0 then
          Phi :=Phi + mol_i[i]*mol_i[j]*(Phi_Phi_ijpi[i,j]);
          //cation-cation-anion
