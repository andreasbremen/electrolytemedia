...
function calc_loggammapipi "Helper function to calculate Gibbs derivative"
  ...
  output Real[nLifun] log_gammapipi;
...
protected
  ...
  Real Fpipi=pipi.calc_Fpipi(T,p,X);
  Real[nLifun,nLifun] C_ij=zeros(nLifun,nLifun);
  Real[nLifun,nLifun] B_ij=zeros(nLifun,nLifun);
  Real[nLifun,nLifun] Phi_ijpipi=pipi.calc_Phi_ijpipi(T,p,X);
  Real Z=0;
...
algorithm
  for i in 1:nLifun loop
    ...
    if datafun[i].z > 0 then
      ...
      ln_gamma[i] :=datafun[i].z^2*Fpipi;
      ...
      for j in 1:nLifun loop
        if datafun[j].z > 0  and j <> i then
          ln_gamma[i] :=ln_gamma[i] + m_i[j]*2*Phi_ijpipi[i, j];
        end if;
      end for;
      //2 cations, 1 anion
      for j in 1:nLifun loop
