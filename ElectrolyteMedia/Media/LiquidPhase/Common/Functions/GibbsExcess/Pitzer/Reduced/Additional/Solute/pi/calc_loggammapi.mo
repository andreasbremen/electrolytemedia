...
function calc_loggammapi "Helper function to calculate Gibbs derivative"
  ...
  output Real[nLifun] log_gammapi;
    ...
    if datafun[i].z > 0 then
      ...
      for j in 1:nLifun loop
        if datafun[j].z > 0  and j <> i then
          ln_gamma[i] :=ln_gamma[i] + m_i[j]*2*Phi_ijpi[i, j];
        end if;
      end for;
      //2 cations, 1 anion
      for j in 1:nLifun loop
